...
import mutagen
import acoustics
import soundfile as sf  # Added for monkeypatch

# Monkeypatch torchaudio.load to use soundfile because Nightly torchaudio is broken on Windows (requires torchcodec)
def patched_load(filepath, **kwargs):
    try:
        # soundfile.read returns (frames, channels)
        ...
        print(f"Warning: Failed to load {filepath}: {e}. Returning silence.")
...
def augment_clips(
    ...
    for i in range(0, len(clip_paths), batch_size):
        ...
        for clip in batch:
            ...
            if clip_sr != sr:
                # Instead of crashing, resample the clip
                resampler = torchaudio.transforms.Resample(orig_freq=clip_sr, new_freq=sr)
                clip_data = resampler(clip_data.float())
                clip_sr = sr
        ...
        augmented_batch = augment2(samples=torch.vstack(augmented_clips).unsqueeze(dim=1).to(torch_device), sample_rate=sr).squeeze(axis=1)
...
def trim_mmap(mmap_path):
    ...
    # Close memmap handles before file operations (required for Windows)
    del mmap_file1
    del mmap_file2
