...
class Model(nn.Module):
    def __init__(self, n_classes=1, input_shape=(16, 96), model_type="dnn",
        ...
        # Check if force_cpu is requested via command line args (hacky access to sys.argv but simple)
        force_cpu = "--force_cpu" in sys.argv
        if force_cpu:
            logging.info("Forcing CPU usage for training/augmentation.")
    ...
    def export_to_onnx(self, output_path, class_mapping="", opset_version=11):
        """
        Export model to ONNX format.
        
        Args:
            output_path: Path to save the ONNX model
            class_mapping: Name for the output
            opset_version: ONNX opset version (default 11 for best Android compatibility)
        """
        obj = self
        # Make simple model for export based on model structure
        if self.n_classes == 1:
            # Save ONNX model - use dynamo=False for legacy TorchScript export
            # which properly respects opset_version for Android compatibility
            torch.onnx.export(self.model.to("cpu"), torch.rand(self.input_shape)[None, ], output_path,
                              output_names=[class_mapping], opset_version=opset_version, dynamo=False)
    ...
    def export_model(self, model, model_name, output_dir):
        ...
        # Save ONNX model - use dynamo=False and opset 11 for HAwake Android compatibility
        # Note: Opset 11 is the recommended version for Android 8+ with ONNX Runtime 1.14.0
        ...
        torch.onnx.export(model_to_save.to("cpu"), torch.rand(self.input_shape)[None, ],
                          os.path.join(output_dir, model_name + ".onnx"), opset_version=11, dynamo=False)
        
        # Save PTH model
        pth_path = os.path.join(output_dir, model_name + ".pth")
        logging.info(f"####\nSaving PTH model as '{pth_path}'")
...
if __name__ == '__main__':
    ...
    # Training data goes directly into output_dir (which is already named after the model)
    positive_train_output_dir = os.path.join(config["output_dir"], "positive_train")
    positive_test_output_dir = os.path.join(config["output_dir"], "positive_test")
    negative_train_output_dir = os.path.join(config["output_dir"], "negative_train")
    negative_test_output_dir = os.path.join(config["output_dir"], "negative_test")
    feature_save_dir = config["output_dir"]
    ...
    if args.augment_clips is True:
        # Determine device logic early for augmentation too
    ...
    if args.train_model is True:
        ...
        for key in ["positive"] + list(config["feature_data_files"].keys()) + ["adversarial_negative"]:
            if key == "positive":
                label_transforms[key] = transform_pos
            else:
                label_transforms[key] = transform_neg
        ...
        # On Windows, using num_workers > 0 often causes PicklingErrors with complex generators
        # Setting to 0 for maximum compatibility.
        if os.name == 'nt':
            n_cpus = 0

        X_train = torch.utils.data.DataLoader(IterDataset(batch_generator),
                                              batch_size=None, num_workers=n_cpus, prefetch_factor=16 if n_cpus > 0 else None)
